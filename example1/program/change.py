...
if __name__ == "__main__":
	session = SparkSession.builder.appName("change").master("local[*]").getOrCreate()
	...
	hierarchical_data_new = dataframereader.option("header","true").option("inferSchema", value = True).csv("/home/maruthimanoj_edu/spark/spark-usecases/Usecase_1/data/sample_data/hierarchical_data_new.csv")
	...
	brandchanges = hierarchical_data \
		...
		.select(hierarchical_data.brand_key, hierarchical_data.brand, hierarchical_data_new.brand).distinct()
	...
	categorychanges =  hierarchical_data \
                ...
                .select(hierarchical_data.brand_key, hierarchical_data.Category, hierarchical_data_new.Category).distinct()
	...
	verticalchanges =  hierarchical_data \
                ...
                .select(hierarchical_data.brand_key, hierarchical_data.Vertical, hierarchical_data_new.Vertical).distinct()
